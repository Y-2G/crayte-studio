...
import { formatDateWithAnomaly, hasHorrorMeta, getHorrorMeta } from '@/lib/horror/utils';
  ...
  const columns = [
    {
      ...
      render: (post: Post) => (
        <div className={styles.titleCell}>
          <Link href={`/admin/posts/${post.id}/edit`} className={styles.titleLink}>
            {post.title}
          </Link>
          <div className={styles.rowActions}>
            ...
            <span className={styles.separator}>|</span>
            <Link href={`/journal/${post.slug}`} className={styles.rowAction} target="_blank">
              表示
            </Link>
          </div>
        </div>
    ...
    },
    {
      ...
      render: (post: Post) => {
        ...
        const isHorrorCategory = post.category === '内部連絡' || post.category === '[REDACTED]';
        return (
          <Link
    ...
    },
    {
      ...
      render: (post: Post) => {
        ...
        const horrorMeta = hasHorrorMeta(post.meta) ? getHorrorMeta(post.meta) : null;
        const isHorrorPost = horrorMeta !== null;

        const statusMap = {
          publish: { label: '公開', className: styles.statusPublish },
          draft: { label: isHorrorPost ? '観察中' : '下書き', className: isHorrorPost ? horrorStyles.horrorText : styles.statusDraft },
    ...
    },
    {
      ...
      sortable: true,
      render: (post: Post) => {
        const dateInfo = formatDateWithAnomaly(post.publishedAt || post.createdAt);
        return (
          <span className={`${styles.date} ${dateInfo.isAnomalous ? horrorStyles.anomalousDate : ''}`}>
            {dateInfo.formatted}
          </span>
        );
      },
    ...
    <div className={styles.container}>
      <div className={styles.bulkActions}>
        ...
        <button
          ...
          onClick={() => {
            const select = document.querySelector<HTMLSelectElement>(`.${styles.bulkSelect}`);
            if (select?.value) {
        ...
        {selectedPosts.length > 0 && (
          <span className={styles.selectedCount}>{selectedPosts.length}件選択中</span>
        )}
      </div>
