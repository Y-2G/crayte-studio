...
「obserq-studio」は、表向きは普通の Web 制作会社のコーポレートサイトだが、裏側（管理画面）にアクセスすると異常なメタデータや状態が徐々に見えてくるホラー体験 ARG（代替現実ゲーム）。
...
| `/services` | サービス | 静的 |
| `/works` | 実績一覧 | 動的（一覧） |
| `/works/[slug]` | 実績詳細 | 動的（詳細） |
| `/journal` | ニュース一覧 | 動的（一覧） |
| `/journal/[slug]` | ニュース詳細 | 動的（詳細） |
| `/staff` | 社員一覧 | 動的（一覧） |
...
- [ ] **T0.1.2** Route Groups の設定
  - `(public)` と `admin` の分離
- [ ] **T0.1.3** 基本レイアウトファイルの作成
...
- [ ] **T0.3.1** CSS 変数の定義
  ...
  - ブレークポイント
  ```
- [ ] **T0.3.2** グローバルスタイルのリセット・設定
- [ ] **T0.3.3** 公開サイト用テーマ変数
- [ ] **T0.3.4** 管理画面用テーマ変数（WP 風）

#### 0.4 共通コンポーネント
- [ ] **T0.4.1** Button コンポーネント
- [ ] **T0.4.2** Card コンポーネント
- [ ] **T0.4.3** Input / Form コンポーネント
- [ ] **T0.4.4** Table コンポーネント（管理画面用）
- [ ] **T0.4.5** Modal コンポーネント
- [ ] **T0.4.6** Loading / Skeleton コンポーネント

#### 0.5 データ層
- [ ] **T0.5.1** サンプルデータ JSON の作成
...
- [ ] **T2.2.1** 統計ウィジェット
  - 投稿数、ページ数、コメント数等
- [ ] **T2.2.2** 最近の投稿ウィジェット
- [ ] **T2.2.3** クイックドラフトウィジェット
- [ ] **T2.2.4** アクティビティウィジェット
...
- [ ] **T2.10.1** 外観設定ページ（プレースホルダー）
- [ ] **T2.10.2** プラグインページ（プレースホルダー）
- [ ] **T2.10.3** ツールページ（エクスポート等）
- [ ] **T2.10.4** 設定ページ
  - サイトタイトル、説明、パーマリンク等
...
#### 3.1 データ統合
- [ ] **T3.1.1** 公開サイト → 管理画面データ連携
- [ ] **T3.1.2** 管理画面での CRUD 操作実装
- [ ] **T3.1.3** 状態管理（Context / Zustand）
...
- [ ] **T3.3.1** 削除済みアイテムの痕跡
  - 例: "この投稿は削除されましたが..."
- [ ] **T3.3.2** 異常なメタデータ
  - 例: `review_status: "under_observation"`
- [ ] **T3.3.3** 見えないコメント
  - 例: コメント数3だが表示は2件
...
- [ ] **T3.4.1** Sealed コンテンツ
  - 例: `status: "sealed"` の投稿
- [ ] **T3.4.2** Rewritten コンテンツ
  - 例: 編集履歴に謎の書き換え
- [ ] **T3.4.3** 削除理由の異常
  - 例: スタッフの `removed_reason: "voluntary_absence"`
...
#### 4.1 レスポンシブ対応
- [ ] **T4.1.1** 公開サイトのモバイル対応
- [ ] **T4.1.2** 管理画面のモバイル対応（サイドバー折りたたみ）

#### 4.2 アクセシビリティ
- [ ] **T4.2.1** キーボードナビゲーション
- [ ] **T4.2.2** スクリーンリーダー対応
- [ ] **T4.2.3** コントラスト・フォーカス表示

#### 4.3 パフォーマンス
- [ ] **T4.3.1** 画像最適化（next/image）
- [ ] **T4.3.2** コード分割・遅延読み込み
- [ ] **T4.3.3** メタデータ・SEO 設定
...
| B (A完了後、並列) | T0.1.2, T0.2.1, T0.3.2 | system-designer |
| C (B完了後、並列) | T0.1.3, T0.2.2, T0.2.3, T0.3.3, T0.3.4 | implementer |
| D (C完了後、並列) | T0.4.1-6, T0.5.1-3 | implementer |
...
| B (A完了後) | T3.2.x, T3.3.x, T3.4.x | domain-analyst + implementer |
| C (B完了後) | T3.5.x | implementer |

#### Phase 4: 仕上げ（並列可能）
| グループ | タスク | 担当 |
|----------|--------|------|
| A (並列) | T4.1.x, T4.2.x | implementer |
| B (A完了後) | T4.3.x | implementer |
| C (B完了後) | T4.4.x | test-engineer |
| D (C完了後) | T4.5.x | docs-writer |
...
- [ ] ESLint 警告なし
- [ ] 未使用のインポート・変数なし
- [ ] コンポーネントの Props 型が明示的
- [ ] エラーハンドリングが適切

### 5.2 UI/UX 品質

- [ ] 公開サイトが「普通のコーポレートサイト」に見える
- [ ] 管理画面が「WordPress 風」に見える
...
| Phase 1 | 5-7 | 7ページ + レイアウト |
| Phase 2 | 8-10 | 12セクション + WP風UI |
